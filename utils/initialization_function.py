...
class InitializationFunction:
    ...
    def uniform_xavier_initialization(self) -> List[float]:
        ...
        for i in range(self.num_outputs):
            weights = np.random.uniform(low=-limit, high=limit, size=(self.num_inputs, self.num_outputs))
    ...
    def normal_xavier_initialization(self) -> List[float]:
        ...
        weights: list = []
        for i in range(self.num_outputs):
            weights.append([np.random.normal(loc=0.0, scale=limit, ) for _ in range(self.num_inputs)])
    ...
    def normal_kaiming_initialization(self) -> List[float]:
        ...
        weights = []
        for i in range(self.num_outputs):
            weights = np.random.uniform(low=0.0, high=limit, size=(self.num_inputs, self.num_outputs))
